...
int main (){
	...
	memset(buffer, 0, WINDOW_WIDTH*WINDOW_HEIGHT*sizeof(Uint32));				//We use memset() to set a value to the pixels. Not ideal as pixels have 4 byts in our case and memset() set the values in our buffer 1 byte at a time
	
	for(int i = 0; i < WINDOW_WIDTH*WINDOW_HEIGHT; i++){					//A for loop going through as meany times as there are pixels
		buffer[i] = 0x0080FFFF;								//Setting the color of the pixel (RGBA) 00=Red 80=Green FF=Blue FF=Alpha
	}
